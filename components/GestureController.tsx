...
export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition, onTwoHandsDetected, onHandRotation }) => {
  ...
  const lastModeRef = useRef<SceneMode>(currentMode);
    ...
    const detectGesture = (landmarks: any[]) => {
      ...
      if (extendedFingers >= 4) {
        ...
        if (openFrames.current > CONFIDENCE_THRESHOLD) {
            if (lastModeRef.current !== SceneMode.CHAOS) {
                lastModeRef.current = SceneMode.CHAOS;
                onModeChange(SceneMode.CHAOS);
            }
        }
        ...
        if (closedFrames.current > CONFIDENCE_THRESHOLD) {
            if (lastModeRef.current !== SceneMode.FORMED) {
                lastModeRef.current = SceneMode.FORMED;
                onModeChange(SceneMode.FORMED);
            }
        }
      } else {
        // Ambiguous
  ...
  return (
    <div className="absolute top-6 right-[8%] z-50 flex flex-col items-end pointer-events-none">
      ...
      <div className="relative w-[18.75vw] h-[14.0625vw] border-2 border-[#ADD8E6] rounded-lg overflow-hidden shadow-[0_0_20px_rgba(173,216,230,0.3)] bg-black">
        {/* Decorative Lines */}
        <div className="absolute inset-0 border border-[#E0F2FE]/20 m-1 rounded-sm z-10"></div>
        ...
        {handPos && (
          <div 
            className="absolute w-2 h-2 bg-[#ADD8E6] rounded-full border border-white"
            style={{
